    ...
    <MotionFadeRightSection>
      <div className="w-full text-black flex  justify-center">
        {ProjectDetails.map((item, index) => (
          <div
            key={index}
            className="mb-[3rem] relative group w-[30%] h-[400px]   bg-[var(--primary)] flex hover:bg-black/50 "
          >
            <div
              className="absolute inset-0 bg-cover w-full bg-center bg-no-repeat "
              style={{ backgroundImage: `url(${item.ProjectImage})` }} 
            ></div>
              <div className=" absolute inset-0 group-hover:bg-black group-hover:opacity-10 transition-opacity duration-3000 z-3 "></div>
